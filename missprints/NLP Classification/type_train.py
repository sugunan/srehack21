...
def load_training_data(sentiment):
    data = open("./traindata/type_data/"+sentiment+"_training_set.txt",encoding="utf8")
    if sentiment == "request":        
        threshold = 0.5
    elif sentiment == "issue":
        threshold = 0.5
    else:
        pass
...
def load_test_data(sentiment):
    data = open("./traindata/type_data/"+sentiment+"_test_set.txt",encoding="utf8")
    return data
...
def main():
    ...
    request_training_data,threshold = load_training_data("request")
    request_training_set = clean_data(request_training_data,threshold)
    
    issue_training_data,threshold = load_training_data("issue")
    issue_training_set = clean_data(issue_training_data,threshold)
    
    
    request_test_data = load_test_data("request")
    request_test_set = clean_data(request_test_data,threshold)
    print(len(request_test_set))
    
    issue_test_data = load_test_data("issue")
    issue_test_set = clean_data(issue_test_data,threshold)
    print(len(issue_test_set))
    

    
    all_data.extend(request_training_set)
    all_data.extend(issue_training_set)
    
    all_data.extend(request_test_set)
    all_data.extend(issue_test_set)
...
def Test_model(test_data, test_labels,words,classes):
    ...
    weights_file = 'type_weights.json' 
